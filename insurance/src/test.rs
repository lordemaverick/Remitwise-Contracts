...
mod tests {
    ...
    fn test_create_policy_indexes_external_ref() {
        ...
        let looked_up =
            client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-A"));
    ...
    fn test_create_policy_none_ref_no_index() {
        ...
        let looked_up =
            client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "anything"));
    ...
    fn test_set_external_ref_reindex() {
        ...
        env.mock_all_auths();
        let result = client.set_external_ref(&owner, &id, &Some(String::from_str(&env, "ref-B")));
        assert_eq!(result, true);

        // Old ref removed
        let old = client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-A"));
        assert_eq!(old, None);

        // New ref indexed
        let new = client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-B"));
    ...
    fn test_set_external_ref_duplicate_rejected() {
        ...
        env.mock_all_auths();
        let result =
            client.try_set_external_ref(&owner, &id1, &Some(String::from_str(&env, "ref-B")));
        assert_eq!(result, Err(Ok(InsuranceError::DuplicateExternalRef)));
    }
    ...
    fn test_set_external_ref_invalid_rejected() {
        ...
        // 129-byte string
        let long_str = "y".repeat(129);
        env.mock_all_auths();
        let result_long =
            client.try_set_external_ref(&owner, &id, &Some(String::from_str(&env, &long_str)));
        assert_eq!(result_long, Err(Ok(InsuranceError::InvalidExternalRef)));
    }
    ...
    fn test_set_external_ref_idempotent() {
        ...
        env.mock_all_auths();
        let result = client.set_external_ref(&owner, &id, &Some(String::from_str(&env, "ref-A")));
        assert_eq!(result, true);
    ...
    fn test_set_external_ref_emits_event() {
        ...
        // Find the ext_ref event
        let expected_action = EVT_EXT_REF_UPDATED;
        let found = events.iter().any(|e| {
            let topics = e.1.clone();
            if topics.len() < 4 {
                return false;
            }
            let t3 = soroban_sdk::Symbol::try_from_val(&env, &topics.get(3).unwrap());
            matches!(t3, Ok(s) if s == expected_action)
        });
        ...
        let payload: ExternalRefUpdatedEvent = soroban_sdk::FromVal::from_val(&env, &evt.2);
    ...
    fn test_set_external_ref_sequential_abc() {
        ...
        assert_eq!(
            client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-B")),
            None
        );
        assert_eq!(
            client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-C")),
            Some(id)
        );
    }
    ...
    fn test_lookup_active_policy() {
        ...
        let looked_up =
            client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-active"));
        assert_eq!(looked_up, Some(id));
    ...
    fn test_lookup_unknown_ref_returns_none() {
        let (env, client, owner) = setup();
        let result = client
            .get_policy_id_by_external_ref(&owner, &String::from_str(&env, "never-registered"));
    ...
    fn test_lookup_stability() {
        ...
        let r1 =
            client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-stable"));
        let r2 =
            client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-stable"));
        let r3 =
            client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-stable"));
    ...
    fn test_lookup_no_stale_after_deactivate() {
        ...
        let result =
            client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-deact"));
        assert_eq!(result, None);
    }
    ...
    fn test_lookup_no_stale_after_archive() {
        ...
        let result =
            client.get_policy_id_by_external_ref(&owner, &String::from_str(&env, "ref-arch"));
        assert_eq!(result, None);
    }
    ...
    proptest! {
        ...
        fn proptest_round_trip(
            ref_str in prop::string::string_regex("[a-zA-Z0-9]{1,64}").unwrap()
        ) {
            let env = Env::default();
            let contract_id = env.register_contract(None, Insurance);
            let client = InsuranceClient::new(&env, &contract_id);
            ...
            let looked_up =
                client.get_policy_id_by_external_ref(&owner, &String::from_str(&env,&ref_str));
...
mod insurance_tests {
    ...
    fn test_set_external_ref_returns_false_for_unknown_policy() {
        ...
        assert_eq!(
            client.try_set_external_ref(&owner, &999u32, &Some(r)),
    ...
    fn test_set_external_ref_returns_false_for_wrong_caller() {
        ...
        assert_eq!(
            client.try_set_external_ref(&other, &id, &Some(r)),
    ...
    fn test_set_external_ref_authorization_non_owner_cannot_set() {
        ...
        let result = client.try_set_external_ref(&attacker, &id, &Some(malicious_ref.clone()));
        assert_eq!(
            result,
            Err(Ok(InsuranceError::Unauthorized)),
            "non-owner must not be able to set external_ref"
        );
    ...
    fn test_set_external_ref_authorization_non_owner_cannot_clear() {
        ...
        let result = client.try_set_external_ref(&attacker, &id, &None);
        assert_eq!(
            result,
            Err(Ok(InsuranceError::Unauthorized)),
            "non-owner must not be able to clear external_ref"
        );
    ...
    fn test_external_ref_updated_event_schema() {
        ...
        assert_eq!(data.policy_id, id, "payload.policy_id mismatch");
        assert_eq!(
            data.new_external_ref,
            Some(new_ref),
            "payload.new_external_ref mismatch"
        );
    ...
    fn test_no_event_emitted_on_failed_operations() {
        ...
        let _ = client.try_set_external_ref(&other, &id, &Some(String::from_str(&env, "X")));
